...
def receive_data():
    """Read CSV lines from serial and unpack into our data arrays."""
    while True:
        try:
...
# GUI SETUP
root = tk.Tk()
root.title("PID Controller & Real-Time Graph (Serial)")
root.state('zoomed')
...
# ───────────────────────────────────────────────────────────
# PID Adjustment UI & Serial Sender
...
tk.Label(left_frame, text="PID Controller Adjustment",
         font=("Arial", 16, "bold"), bg="lightgray").pack(pady=(0,10))

entry_widgets = {}
for param in pid_values:
    frame = tk.Frame(left_frame, bg="lightgray")
    frame.pack(fill=tk.X, pady=2)
    tk.Label(frame, text=f"{param}:", width=10, anchor='w', bg="lightgray").pack(side=tk.LEFT)
...
tk.Button(left_frame, text="Pause/Resume", command=toggle_pause, bg="lightblue").pack(pady=10)
